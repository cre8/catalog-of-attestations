...
| trust_anchor | Location of the trust anchor for verification | URI | `https://raw.githubusercontent.com/cre8/catalog-of-attestations/main/trust-lists/gym-members.jws` |
...
```
https://raw.githubusercontent.com/cre8/catalog-of-attestations/main/trust-lists/gym-members.jws
```

This is a **JWS-signed trust list**. The verification key is embedded in the attestation definition's `trustedAuthorities[].verificationMethod` property.
...
1. Load the attestation definition and extract `trustedAuthorities[]`.
2. Fetch the JWS-signed trust list from the `value` URI.
3. Verify the JWS signature using the `verificationMethod.publicKeyJwk`.
4. Parse the JWS payload to obtain the LoTE.
5. Locate the issuer in the `TrustedEntitiesList`.
6. Verify that the issuer's status allows for valid attestation issuance.
7. Use the issuer's public key or certificate to verify the attestation signature.
